...
for fold in folds:
    ...
    p_5 = (HE_fold['p_5'] + KI67_fold['p_5']) / 2
    p_6 = (HE_fold['p_6'] + KI67_fold['p_6']) / 2
    p_7 = (HE_fold['p_7'] + KI67_fold['p_7']) / 2
    p_8 = (HE_fold['p_8'] + KI67_fold['p_8']) / 2
    Y = HE_fold['Y']
    Y_hat = [None] * len(Y)
    for i in range(len(p_0)):
        # get the max probability
        max_prob = max(p_0[i], p_1[i], p_2[i], p_3[i], p_4[i], p_5[i], p_6[i], p_7[i], p_8[i])
    ...
    Merged_HE_KI67_fold = pd.DataFrame({
        ...
        'p_8': p_8})
    # define save path
    save_path = f'/local/data1/chrsp39/CBTN_Histology_Multi_Stain/models/CLAM/eval_results/9_class/EVAL_9_class_Late_Fusion_PA_HE_KI67_small_clam_sb_conch_v1_5/{fold}.csv'
