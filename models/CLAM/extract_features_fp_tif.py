...
from dataset_modules.dataset_h5_tif import Dataset_All_Bags, Whole_Slide_Bag_FP
...
def main(cfg:DictConfig):
	...
	if cfg.model_name in ['resnet50', 'uni', 'conch_v1', 'hipt', 'prov-gigapath', 'uni2-h', 'conch_v1_5']:
	...
	for bag_candidate_idx in tqdm(range(total)):
		...
		# For TIF files, prefer PIL Image.open since OpenSlide has issues with these files
		if cfg.slide_ext.lower() in ['.tif']:
			print(f"TIF file detected, trying PIL first for {slide_id}")
			try:
				wsi = Image.open(slide_file_path)
				print(f"Successfully opened {slide_id} with PIL Image.open")
			except Exception as e:
				print(f"Failed to open {slide_id} with PIL Image.open: {e}")
				try:
					wsi = openslide.open_slide(slide_file_path)
					print(f"Successfully opened {slide_id} with openslide (fallback)")
				except Exception as e2:
					print(f"Failed to open {slide_id} with openslide: {e2}")
					print(f"Skipping {slide_id} - unable to open with either method")
					continue
		else:
			# For non-TIF files, try OpenSlide first, then PIL
			try:
				wsi = openslide.open_slide(slide_file_path)
				print(f"Successfully opened {slide_id} with openslide")
			except Exception as e:
				print(f"Failed to open {slide_id} with openslide: {e}")
				try:
					wsi = Image.open(slide_file_path)
					print(f"Successfully opened {slide_id} with PIL Image.open")
				except Exception as e2:
					print(f"Failed to open {slide_id} with PIL Image.open: {e2}")
					print(f"Skipping {slide_id} - unable to open with either method")
					continue
		...
		if cfg.model_name in ['virchow', 'virchow2']:
			...
			try:
				if os.path.exists(os.path.join(cfg.feat_dir, 'pt_files_class_tokens_batch')):
					shutil.rmtree(os.path.join(cfg.feat_dir, 'pt_files_class_tokens_batch'))
			except Exception as e:
				print(f"Warning: Could not remove pt_files_class_tokens_batch: {e}")
			
			try:
				if os.path.exists(os.path.join(cfg.feat_dir, 'pt_files_patch_tokens_batch')):
					shutil.rmtree(os.path.join(cfg.feat_dir, 'pt_files_patch_tokens_batch'))
			except Exception as e:
				print(f"Warning: Could not remove pt_files_patch_tokens_batch: {e}")

	# delete directory that are not needed
	try:
		if os.path.exists(os.path.join(cfg.feat_dir, 'h5_files')):
			shutil.rmtree(os.path.join(cfg.feat_dir, 'h5_files'))
	except Exception as e:
		print(f"Warning: Could not remove h5_files directory: {e}")
