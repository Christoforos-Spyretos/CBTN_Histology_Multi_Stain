...
def get_histology_information(json_information_file):
    ...
    acquisition_info_dict = {
        ...
        "dimension_x": None,  # mpp-x
        "dimension_y": None,  # mpp-y
        "scanner": None,  # vendor
        "pixels_x": None,
        "pixels_y": None
    }
...
# %% 
#through radiology data
rad_sessions = {}

# Loop through every tumor type
for tumor_type_key, tumor_type_value in tumor_types.items():
    tumor_type_path = os.path.join(radiology_dataset_path, tumor_type_value)
    # Check if the tumor type directory exists
    if os.path.exists(tumor_type_path):
        # Loop through each SUBJECTS directory
        subjects_path = os.path.join(tumor_type_path, "SUBJECTS")
        for subject in os.listdir(subjects_path):
            subject_path = os.path.join(subjects_path, subject)
            # Check if the subject directory exists
            if os.path.isdir(subject_path):
                # Loop through each SESSIONS directory
                sessions_path = os.path.join(subject_path, "SESSIONS")
                for session in os.listdir(sessions_path):
                    if "B_brain" in session:
                        session_name = os.path.basename(session)
                        session_name = session_name.split("d")[0]
                        radiology_folder = os.path.join(tumor_type_path, "SUBJECTS")
                        # Add the session_name, tumor_type, and radiology folder to the dictionary
                        rad_sessions[session_name] = {
                            "tumor_type": tumor_type_key,
                            "radiology_folder": radiology_folder,
                            "histology_folder": None
                        }
...
for subject_id in subjects:
    subject_sessions_path = os.path.join(histology_dataset_path, "SUBJECTS", subject_id, "SESSIONS")

    if os.path.exists(subject_sessions_path):
        sessions = [session for session in os.listdir(subject_sessions_path) if os.path.isdir(os.path.join(subject_sessions_path, session))]
        for session in sessions:
            # Check if the session exists in the rad_sessions dictionary
            if session in rad_sessions:
                tumor_type = rad_sessions[session]["tumor_type"]
                radiology_folder = rad_sessions[session]["radiology_folder"]
            else:
                tumor_type = None
                radiology_folder = None

            # Add the tumor type and radiology folder (if available) to the session in hist_sessions dictionary
            histology_folder = "/run/media/chrsp39/CBNT_v2/Datasets/CBTN_v2/HISTOLOGY/SUBJECTS/" + subject_id
            hist_sessions[session] = {
                "tumor_type": tumor_type,
                "radiology_folder": radiology_folder,
                "histology_folder": histology_folder
            }

external_sample_IDs = hist_sessions
...
clinical_file_from_portal_patients_info = pd.read_excel(
    clinical_file_from_portal_path, 
...
clinical_file_from_portal_histological_info = pd.read_excel(
    clinical_file_from_portal_path, 
    sheet_name="Histological Diagnoses", 
    index_col="External Sample Id"
)
...
biospecimen_file_from_portal = biospecimen_file_from_portal[
    biospecimen_file_from_portal["External Id"].isin(
        clinical_file_from_portal_patients_info.index
    )
]
...
"""
Open the clinical_file_from_portal and get the unique external sample ids.
For the unique external sample ids, extract the information that is available from the 
clinical_file_from_portal_histological_info (sex, diagnosis, age_at_diagnosis, overall survival)
Also get the information regarding the age_at_sample_acquisition alligned with the tumor_descriptor 
from the biospecimen_file_from_portal. Get progression_free_survival information (if avvailable) 
from the detailed clinical files

NB! 
There are external sample IDs which have multiple versions in the Histological Diagnoses list. 
However, only one version of them hasthe information. Thus, one needs to drop all those cases 
to avoid errors in the code later
"""

aus_len = len(str(len(clinical_file_from_portal_histological_info.index.unique())))
for idx, external_sample in enumerate(clinical_file_from_portal_histological_info.index.unique()):
    print(
        f"Working on subject {idx+1:0{aus_len}d}/{len(clinical_file_from_portal_histological_info.index.unique())} \r",
        end="",
    )
    if external_sample in external_sample_IDs.keys():
        # build entry in the all_information dictionary
        all_information[external_sample] = get_empty_overall_subject_info()

        # get information from the clinical_file_from_portal_histological_info
        for ikey, ykey in zip(
            ["subject_ID", "diagnosis", "age_at_diagnosis", "tumor_location"],
            ["External Id", "Histological Diagnosis (Source Text)", "Age at Diagnosis (Days)", "Histological Tumor Location"],
        ):

            try:
                # Use iloc to retrieve the first value for the specific column and row (external_sample)
                value = clinical_file_from_portal_histological_info.loc[external_sample, ykey].iloc[0]
                all_information[external_sample][ikey] = value
            except:
                continue

        # Check if external_sample_id exists in biospecimen_file_from_portal.index
        if external_sample in biospecimen_file_from_portal.index:
            # get information from the biospecimen_file_from_portal
            aus_info = (
                biospecimen_file_from_portal.loc[[external_sample]][
            ...
            # Check if any values exist in the DataFrame before accessing the first value
            if not aus_info.empty:
                all_information[external_sample]["age_at_sample_acquisition"] = aus_info["Age at Sample Acquisition"].iloc[0]
                all_information[external_sample]["tumor_descriptor"] = aus_info["Tumor Descriptor"].iloc[0]
            else:
                all_information[external_sample]["age_at_sample_acquisition"] = None
                all_information[external_sample]["tumor_descriptor"] = None


        # get information from the clinical_file_from_portal_patients
        subject_id = all_information[external_sample]["subject_ID"]
        if subject_id in clinical_file_from_portal_patients_info.index:
            all_information[external_sample]["ethnicity"] = clinical_file_from_portal_patients_info.loc[subject_id, "Ethnicity"]
            all_information[external_sample]["gender"] = clinical_file_from_portal_patients_info.loc[subject_id, "Gender"]
            all_information[external_sample]["survival"] = clinical_file_from_portal_patients_info.loc[subject_id, "Age at the Last Vital Status (Days)"]
        else:
            all_information[external_sample]["gender"] = None
            all_information[external_sample]["survival"] = None
            all_information[external_sample]['ethnicity'] = None

for external_sample in all_information:
    all_information[external_sample]['type_of_session'] = 'histology'
    all_information[external_sample]['status']= 'None'

# %%
# and finally check that all the files that have been included
missing_info = []
for external_sample in external_sample_IDs.keys():
    if external_sample not in all_information.keys():
        missing_info.append(external_sample)

if len(missing_info) != 0:
    print(
        f"Missing clinical information for {len(missing_info)} external_sample. Adding information..."
    )
    for external_sample in missing_info:
        all_information[external_sample] = get_empty_overall_subject_info()

# %% LOOP THROUGH THE UNIQUE external_sample_ID AND SCRAPE THE DATA (from the dataset folders)
for external_sample_idx, (external_sample, subj_path_info) in enumerate(external_sample_IDs.items()):
    histology_folder = subj_path_info["histology_folder"]

    if histology_folder:
        session_iter = glob.glob(
            os.path.join(histology_folder, "SESSIONS", "*/")
        )
        ...
        if len(session_iter) != 0:
            ...
            for session_idx, session in enumerate(session_iter):
                ...
                for file_idx, file in enumerate(files_iter):
                    ...
                    # add stain name in the dictionary if not present
                    if stain_type not in all_information[session_name]:
                        all_information[session_name][stain_type] = []

                    # check if histo_information already exists for this session and stain type
                    if histo_information not in all_information[session_name][stain_type]:
                        # save the information in the all_information
                        all_information[session_name][stain_type].append(histo_information)
...
with open(
    "/run/media/chrsp39/CBNT_v2/Datasets/CBTN_v2/SUMMARY_FILES/summary_scraped_histology_dataset.json",
    "w",
) as fp:
    json.dump(all_information, fp, sort_keys=True)

# %% BUILD THE EXTERNAL SAMPLE LEVEL CSV FILE
import csv
import json

output_file_path = "/run/media/chrsp39/CBNT_v2/Datasets/CBTN_v2/SUMMARY_FILES/HISTOLOGY/CBTN_histology_summary.csv"
...
with open(output_file_path, mode='w', newline='') as file:
    ...
    for session_name, session_info in all_information.items():
        for stain_type, stain_info in session_info.items():
            if isinstance(stain_info, list): 
                for histo_info in stain_info:

                    # convert inches to millimeters for dimension_x and dimension_y (multiply with 25.4)
                    dimension_x_mm = float(histo_info.get('dimension_x', 0)) * 25.4
                    dimension_y_mm = float(histo_info.get('dimension_y', 0)) * 25.4
                    # dimension_x_value = histo_info.get('dimension_x', 0)
                    # dimension_y_value = histo_info.get('dimension_y', 0)

                    # if dimension_x_value != 'Not available' and dimension_x_value is not None:
                    #     dimension_x_mm = float(dimension_x_value) * 25.4
                    # else:
                    #     dimension_x_mm = dimension_x_value

                    # if dimension_y_value != 'Not available' and dimension_y_value is not None:
                    #     dimension_y_mm = float(dimension_y_value) * 25.4
                    # else:
                    #     dimension_y_mm = dimension_y_value

                    row_data = {
                        'session_name': session_name,
                        'image_type': stain_type,
                        'subjectID': session_info.get('subject_ID', None),
                        'diagnosis': session_info.get('diagnosis', None),
                        'age_at_diagnosis': session_info.get('age_at_diagnosis', None),
                        'tumor_location': session_info.get('tumor_location', None),
                        'age_at_sample_acquisition': session_info.get('age_at_sample_acquisition', None),
                        'tumor_descriptor': session_info.get('tumor_descriptor', None),
                        'ethnicity': session_info.get('ethnicity', None),
                        'gender': session_info.get('gender', None),
                        'survival': session_info.get('survival', None),
                        'type_of_session': session_info.get('type_of_session', None),
                        'session_status': session_info.get('status', None),
                        'magnification': histo_info.get('magnification', None),
                        'dimension_x': histo_info.get('dimension_x', None),
                        'dimension_y': histo_info.get('dimension_y', None),
                        'dimension_z': 0,  # default value for dimension_z
                        'scanner': histo_info.get('scanner', None),
                        'pixels_x': histo_info.get('pixels_x', None),
                        'pixels_y': histo_info.get('pixels_y', None),
                        'pixels_z': 0  # default value for pixels_z
                    }
