...
for split_name in ['train', 'test']:
    ...
    # plot_pca(features_matrix, labels, f'PCA of Merged H&E & Ki-67 Features ({split_name})', 
    #          os.path.join(pca_output_dir, f'PCA_raw_Merged_HE_KI67_features_5_class_{split_name}.png'), labels_to_include)
    plot_tsne(features_matrix, labels, f't-SNE of Merged H&E & Ki-67 Features ({split_name})', 
              os.path.join(tsne_output_dir, f'tSNE_raw_Early_Fusion_HE_KI67_features_5_class_{split_name}.png'), labels_to_include)
...
for split_name in ['train', 'test']:
    ...
    path_to_early_fusion_features = f'/local/data3/chrsp39/CBTN_v2/SUBJECT_LEVEL_ATTENTION_Early_Fusion_HE_KI67/70%_split/5_class/features/conch_v1_5/{split_name}/pt_files/fold_12'
    ...
    # plot_pca(features_matrix, label_array, f'PCA of Learned Early Fusion H&E & Ki-67 Features ({split_name})', os.path.join(pca_output_dir, f'PCA_learned_Early_Fusion_features_5_class_{split_name}.png'), labels_to_include)
    plot_tsne(features_matrix, label_array, f't-SNE of Learned Early Fusion H&E & Ki-67 Features ({split_name})',
              os.path.join(tsne_output_dir, f'tSNE_learned_Early_Fusion_HE_KI67_features_5_class_{split_name}.png'), labels_to_include)
