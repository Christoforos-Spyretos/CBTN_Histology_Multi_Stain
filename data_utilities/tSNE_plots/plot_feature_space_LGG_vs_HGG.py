...
dataset_csv = '/local/data3/chrsp39/CBTN_v2/CSVs/Merged_HE_KI67_LGG_vs_HGG_dataset.csv'
...
for split_name in ['train', 'test']:
    ...
    plot_tsne(features_matrix, labels, f't-SNE of H&E Features ({split_name})',
              os.path.join(tsne_output_dir, f'tSNE_raw_HE_features_LGG_vs_HGG_{split_name}.png'))
...
for split_name in ['train', 'test']:
    ...
    plot_tsne(features_matrix, labels, f't-SNE of Ki-67 Features ({split_name})',
              os.path.join(tsne_output_dir, f'tSNE_raw_KI67_features_LGG_vs_HGG_{split_name}.png'))

# %% t-SNE FOR LEARNED H&E FEATURES
for split_name in ['train', 'test']:
    print(f"\n=== Learned H&E features — {split_name.upper()} ===")
    path = f'/local/data3/chrsp39/CBTN_v2/SUBJECT_LEVEL_ATTENTION_HE/70%_split/LGG_vs_HGG/features/conch_v1_5/{split_name}/pt_files/fold_9'
    ...
    plot_tsne(features_matrix, labels, f't-SNE of Learned H&E Features ({split_name})',
              os.path.join(tsne_output_dir, f'tSNE_learned_HE_features_LGG_vs_HGG_{split_name}.png'))

# %% t-SNE FOR LEARNED Ki-67 FEATURES
for split_name in ['train', 'test']:
    print(f"\n=== Learned Ki-67 features — {split_name.upper()} ===")
    path = f'/local/data3/chrsp39/CBTN_v2/SUBJECT_LEVEL_ATTENTION_KI67/70%_split/LGG_vs_HGG/features/conch_v1_5/{split_name}/pt_files/fold_9'
    features_matrix, labels, _ = collect_features_and_labels(df, path, labels_to_include, split_case_ids[split_name])
    plot_tsne(features_matrix, labels, f't-SNE of Learned Ki-67 Features ({split_name})',
              os.path.join(tsne_output_dir, f'tSNE_learned_KI67_features_LGG_vs_HGG_{split_name}.png'))
...
for split_name in ['train', 'test']:
    ...
    path = f'/local/data3/chrsp39/CBTN_v2/SUBJECT_LEVEL_ATTENTION_Early_Fusion_HE_KI67/70%_split/LGG_vs_HGG/features/conch_v1_5/{split_name}/pt_files/fold_9'
    features_matrix, labels, _ = collect_features_and_labels(df, path, labels_to_include, split_case_ids[split_name])
    plot_tsne(features_matrix, labels, f't-SNE of Learned Early Fusion H&E + Ki-67 Features ({split_name})',
              os.path.join(tsne_output_dir, f'tSNE_learned_Early_Fusion_HE_KI67_features_LGG_vs_HGG_{split_name}.png'))

# %% t-SNE FOR INTERMEDIATE FUSION — CONCATENATION
for split_name in ['train', 'test']:
    print(f"\n=== Concatenation features — {split_name.upper()} ===")
    path = f'/local/data3/chrsp39/CBTN_v2/SUBJECT_LEVEL_CONCATENATION_HE_KI67/70%_split/LGG_vs_HGG/features/conch_v1_5/{split_name}/fold_9/pt_files'
    features_matrix, labels, _ = collect_features_and_labels(df, path, labels_to_include, split_case_ids[split_name])
    plot_tsne(features_matrix, labels, f't-SNE of Concatenation H&E + Ki-67 Features ({split_name})',
              os.path.join(tsne_output_dir, f'tSNE_learned_Concatenation_HE_KI67_features_LGG_vs_HGG_{split_name}.png'))

# %% t-SNE FOR INTERMEDIATE FUSION — ELEMENT-WISE MULTIPLICATION
for split_name in ['train', 'test']:
    print(f"\n=== Element-wise Multiplication features — {split_name.upper()} ===")
    path = f'/local/data3/chrsp39/CBTN_v2/SUBJECT_LEVEL_ELEMENT_WISE_MULT_HE_KI67/70%_split/LGG_vs_HGG/features/conch_v1_5/{split_name}/fold_9/pt_files'
    features_matrix, labels, _ = collect_features_and_labels(df, path, labels_to_include, split_case_ids[split_name])
    plot_tsne(features_matrix, labels, f't-SNE of Element-wise Multiplication H&E + Ki-67 Features ({split_name})',
              os.path.join(tsne_output_dir, f'tSNE_learned_ElemWiseMult_HE_KI67_features_LGG_vs_HGG_{split_name}.png'))

# %% t-SNE FOR INTERMEDIATE FUSION — H&E-GUIDED CROSS ATTENTION
for split_name in ['train', 'test']:
    print(f"\n=== H&E-guided Cross Attention features — {split_name.upper()} ===")
    path = f'/local/data3/chrsp39/CBTN_v2/SUBJECT_LEVEL_CROSS_ATTENTION_HE_inform_KI67/70%_split/LGG_vs_HGG/features/conch_v1_5/{split_name}/fold_9/pt_files'
    features_matrix, labels, _ = collect_features_and_labels(df, path, labels_to_include, split_case_ids[split_name])
    plot_tsne(features_matrix, labels, f't-SNE of H&E-guided Cross Attention Features ({split_name})',
              os.path.join(tsne_output_dir, f'tSNE_learned_CrossAttn_HE_inform_KI67_features_LGG_vs_HGG_{split_name}.png'))

# %% t-SNE FOR INTERMEDIATE FUSION — Ki-67-GUIDED CROSS ATTENTION
for split_name in ['train', 'test']:
    print(f"\n=== Ki-67-guided Cross Attention features — {split_name.upper()} ===")
    path = f'/local/data3/chrsp39/CBTN_v2/SUBJECT_LEVEL_CROSS_ATTENTION_KI67_inform_HE/70%_split/LGG_vs_HGG/features/conch_v1_5/{split_name}/fold_9/pt_files'
    features_matrix, labels, _ = collect_features_and_labels(df, path, labels_to_include, split_case_ids[split_name])
    plot_tsne(features_matrix, labels, f't-SNE of Ki-67-guided Cross Attention Features ({split_name})',
              os.path.join(tsne_output_dir, f'tSNE_learned_CrossAttn_KI67_inform_HE_features_LGG_vs_HGG_{split_name}.png'))
